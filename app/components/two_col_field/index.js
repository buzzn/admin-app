...
class TwoColField extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      !isEqual(
        prevProps.validationRules[camelCase(prevProps.name)],
        this.props.validationRules[camelCase(this.props.name)],
      )
    ) {
    ...
    return (
      <Row className="fieldgroup">
        ...
        <Col xs="6" className={`${this.props.editMode ? 'editValue' : 'fieldvalue grey-underline'}`}>
          <Field
            {...this.props}
            field={this.props.field || this.props.validationRules[camelCase(this.props.name)]}
